...
public class Inventory : MonoBehaviour
{
    ...
    public bool RemoveItem(int index)
    {
        ...
        if (playerState.inventoryItems[index] == playerItemHandler.currentItem)
        {
    ...
    public bool RemoveItemFromInstance(ItemInstance currItem)
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            if (items[i] == currItem)
            {
                ...
                {
                    if (playerItemHandler.currentItem == currItem)
                    {
                        playerItemHandler.ClearHolding();
                    }
                }
    ...
    public void RemoveFromQuickSlot(int index)
    {
        ...
        for (int i = 0; i < playerState.quickSlotIndexs.Length; i++)
        {
            if (playerState.quickSlotIndexs[i] == index)
            {
    ...
    private void SwapQuickSlots(int indexA, int indexB)
    {
        ...
        for (int i = 0; i < playerState.quickSlotIndexs.Length; i++)
        {
            if (playerState.quickSlotIndexs[i] == indexA)
            {
                isIndexA = i;
            }
            if (playerState.quickSlotIndexs[i] == indexB)
            {
                isIndexB = i;
            }
        }
        ...
        if (isIndexA >= 0)
        {
            playerState.quickSlotIndexs[isIndexA] = indexB;
        }
        if (isIndexB >= 0)
        {
            playerState.quickSlotIndexs[isIndexB] = indexA;
        }
    }
}
